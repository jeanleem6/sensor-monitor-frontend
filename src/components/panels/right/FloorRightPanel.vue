...
const itemBase   = 'flex items-center gap-2 px-2 py-1.5 rounded cursor-pointer text-cyan-100 transition-colors hover:bg-primary/15'
...
<template>
  ...
  <BasePanel title="温湿度监测">
    ...
    <EChart v-else :option="scatterOption" height="190px" @chart-click="(p) => p?.data?.name && store.enterRoom(p.data.name)" />
      ...
      >
        ...
        <div class="flex-1 h-1.5 rounded bg-cyan-400/10 overflow-hidden">
          <div
            ...
            :class="{ 'bg-emerald-400': r.status === 'normal', 'bg-amber-300': r.status === 'warning', 'bg-rose-400': r.status === 'danger' }"
    ...
    <template v-else>
      ...
      <div class="mt-1 overflow-y-auto max-h-41.75 rounded border border-primary/10 bg-primary/4 p-1">
        <EChart
          ...
          @chart-click="(p) => { const name = roomNames[p?.data?.[1]]; if (name) store.enterRoom(name) }"
        />
      </div>
      <div class="mt-2 pt-2 border-t border-primary/20 flex items-center justify-between text-sm text-cyan-200/60">
        <span class="flex items-center gap-1">
          ...
          当前活跃房间
          <span class="font-mono text-cyan-100 ml-1">{{ irRows.filter((r) => r.occupied).length }} / {{ roomNames.length }}</span>
        </span>
        <span class="flex items-center gap-1 text-cyan-200/50">
