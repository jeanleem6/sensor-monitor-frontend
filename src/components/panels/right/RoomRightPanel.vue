...
<template>
  ...
  <BasePanel title="温度监测">
      ...
      <div class="flex-1 text-sm space-y-1">
        <div class="flex justify-between text-cyan-200/70">
          ...
          <span class="flex items-baseline gap-1.5">
            <span :class="['font-mono font-bold text-base', tempStatus === 'danger' ? 'text-rose-400' : tempStatus === 'warning' ? 'text-amber-300' : 'text-cyan-50']">
              {{ tempCurrent.toFixed(1) }}°C
            </span>
            <span :class="['text-xs', tempStatus === 'danger' ? 'text-rose-400' : tempStatus === 'warning' ? 'text-amber-300' : 'text-emerald-300']">
    ...
    <div class="mt-2 pt-2 border-t border-primary/20 flex items-center justify-center gap-1 text-sm text-primary/80 hover:text-primary cursor-pointer" @click="tempModal = true">
      <Icon icon="lucide:history" class="text-sm" /><span>查看历史数据</span><Icon icon="lucide:chevron-right" class="text-sm" />
    </div>
  </BasePanel>
  ...
  <BasePanel title="湿度监测" class="mt-5">
    <div class="flex items-center gap-3 mb-2">
      ...
      <div class="flex-1 text-sm space-y-1">
        <div class="flex justify-between text-cyan-200/70">
          <span>当前湿度</span>
          <span class="flex items-baseline gap-1.5">
            <span :class="['font-mono font-bold text-base', humStatus === 'warning' ? 'text-amber-300' : 'text-cyan-50']">{{ humCurrent }}%</span>
            <span :class="['text-xs', humStatus === 'warning' ? 'text-amber-300' : 'text-emerald-300']">{{ humStatus === 'warning' ? '异常' : '正常' }}</span>
          </span>
        </div>
        <div class="flex justify-between text-cyan-200/60">
    ...
    <div class="mt-2 pt-2 border-t border-primary/20 flex items-center justify-center gap-1 text-sm text-primary/80 hover:text-primary cursor-pointer" @click="humModal = true">
      <Icon icon="lucide:history" class="text-sm" /><span>查看历史数据</span><Icon icon="lucide:chevron-right" class="text-sm" />
    </div>
  </BasePanel>
  ...
  <BasePanel title="二氧化碳监测" class="mt-5">
    <div class="flex items-center gap-3 mb-2">
      ...
      <div class="flex-1 text-sm space-y-0.5">
        <div class="flex justify-between text-cyan-200/70">
          <span>当前浓度</span>
          <span :class="['font-mono font-bold text-base', co2Status === 'danger' ? 'text-rose-400' : co2Status === 'warning' ? 'text-amber-300' : 'text-cyan-50']">
            {{ co2Current }} ppm
          </span>
        </div>
    ...
    <div class="mt-2 pt-2 border-t border-primary/20 flex items-center justify-center gap-1 text-sm text-primary/80 hover:text-primary cursor-pointer" @click="co2Modal = true">
      <Icon icon="lucide:history" class="text-sm" /><span>查看历史数据</span><Icon icon="lucide:chevron-right" class="text-sm" />
    </div>
  </BasePanel>

  <!-- 历史 Modals -->
  <MetricHistoryModal v-model="tempModal" title="温度历史数据"    icon="mdi:thermometer-lines" :stat-defs="TEMP_STATS" :stat-values="tempHistoryStat" :chart-option="tempHistoryOpt" />
  <MetricHistoryModal v-model="humModal"  title="湿度历史数据"    icon="mdi:water-percent"    :stat-defs="HUM_STATS"  :stat-values="humHistoryStat"  :chart-option="humHistoryOpt"  />
  <MetricHistoryModal v-model="co2Modal"  title="二氧化碳历史数据" icon="mdi:molecule-co2"      :stat-defs="CO2_STATS"  :stat-values="co2HistoryStat"  :chart-option="co2HistoryOpt"  />
</template>
