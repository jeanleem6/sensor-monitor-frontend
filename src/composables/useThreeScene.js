...
export function createThreeScene(container, store) {
  ...
  // 渲染出来发暗发灰。这里只把近中性（低饱和度）的浅灰/白材质统一抬到一个固定的中性灰，
  ...
  // 仅对"近乎纯中性、且属最亮一档"的灰下手：把漫反射重设为统一灰度，并叠加极少量自发光
  // （emissive 不受场景光照影响，能把背光/侧光面也托起来消除发灰，但过量会让整片墙体发白刺眼）。
  // 取值经过收敛：色相重设到 0.25 的中性灰（不是纯白——配合调亮后的灯光，过亮会刺眼），
  // emissive 仅 0.01 做极轻微补光；整体与场景灯光、房间配色协调，人眼看久不累。
